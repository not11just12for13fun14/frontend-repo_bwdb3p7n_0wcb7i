...
import { Server, Swords, Zap, TreePine } from 'lucide-react'
          ...
          {packs.map((pack, i) => (
            <motion.div key={pack.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="inline-flex items-center gap-2 rounded-lg border border-slate-700/60 bg-black/40 px-2 py-1 text-xs text-slate-300/90">
                    <span className="inline-flex size-2 rounded-sm bg-emerald-400" />
                    Online: {pack.online}
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {pack.tags.map(t => (
                      <span key={t} className="text-xs rounded-lg border border-slate-700/60 bg-slate-900/60 px-2 py-1 text-slate-300">{t}</span>
                    ))}
                  </div>
                </div>
              ...
              <div className="mt-6 flex items-center justify-between">
                ...
                <button className="rounded-lg px-3 py-2 text-sm font-medium bg-emerald-500 text-slate-900 hover:bg-emerald-400 transition">Copy</button>
              </div>
            </motion.div>
          ))}
        </div>
